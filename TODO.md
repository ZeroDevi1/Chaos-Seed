...
## 音乐（搜索 / 登录 / 下载）
- 歌曲下载链路（core/daemon/WinUI3）已接入：QQ 音乐（QQ/微信扫码登录、刷新 Cookie、音质选择）；酷狗/网易云（可配置 baseUrl）；酷我；支持按单曲/专辑/歌手搜索与批量下载；下载任务由 daemon 执行并可轮询进度/取消
- 下载体验增强：任务持久化（重启恢复/断点续传）、失败重试策略、速度限制与并发上限
- 元数据与落盘：封面/ID3 tag 写入、文件命名模板与冲突策略（覆盖/自动重命名）
- Provider 健康检测与 fallback：baseUrl 探活、网易多 baseUrl 轮询与降级策略、可观测性（日志/错误聚合）
