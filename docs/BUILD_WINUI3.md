...
- `cargo xtask build-winui3 --release` 不再让 `chaos-daemon.exe` / `chaos_ffi.dll` 在编译期绑定 PyO3。
  - 无论当前机器是否安装/配置 Python，daemon/FFI 都应能正常构建与启动
  - 若存在 `third_party/voicelab_py_env/.venv/Scripts/python.exe` 或显式设置 `CHAOS_TTS_PYTHON_EXE`，运行时 TTS/语音聊天会优先复用该 Python
  - 若没有 Python 环境，直播/分类/弹幕/歌词等基础功能不受影响；只有真正调用 TTS/语音聊天时才会返回对应错误

## 运行（开发/调试）

1. 运行 `chaos-winui3` 输出目录下的 WinUI 3 可执行文件（当选择 daemon 后端时会自动启动 `chaos-daemon.exe`）
   - 设置页可选择直播后端：自动 / FFI / daemon
   - 设置页可选择歌词后端：自动 / FFI / daemon
   - 设置页直播相关选项：
     - “直播：默认全屏播放”：开启后进入播放会先在应用内全屏覆盖，然后再切系统全屏（隐藏导航/标题栏）
     - “直播：全屏切换延迟(ms)（调试）”：用于调试/缓解全屏切换时的卡顿；建议从 `350ms` 起逐步调大观察
2. 在“直播”页输入直播间地址（URL 或带平台前缀的输入）
3. 点击“解析”进入播放：
...
> 说明：`BtbN/FFmpeg-Builds` 的 release 资产命名近来从 `*-latest-*` 逐步切换为 `ffmpeg-N-*` / `ffmpeg-n8.0.1-...-8.0.zip` 这类格式；仓库内脚本与 WinUI3 运行时安装逻辑已兼容新旧两种命名及统一的 `checksums.sha256` 校验文件。
