...
fn handle_json(
    ...
    if cmd.starts_with("DANMU_MSG") {
        ...
        if let Some(extra_str) = info0
            ...
            .and_then(|x| x.as_str())
            && let Ok(extra_v) = serde_json::from_str::<serde_json::Value>(extra_str)
        {
            let content = extra_v
                .get("content")
                .and_then(|x| x.as_str())
            ...
            if let Some(emots) = extra_v.get("emots").and_then(|x| x.as_object())
                && let Some((_k, v0)) = emots.iter().next()
                && let Some(url) = v0.get("url").and_then(|x| x.as_str())
            {
            ...
            if let Some(unique) = extra_v.get("emoticon_unique").and_then(|x| x.as_str())
                && let Some(meta) = emoticons.get(unique)
            {
                let c = DanmakuComment {
                    text: "".to_string(),
    ...
    if let Some(dm_v2) = v
        ...
        .and_then(|x| x.as_str())
        .filter(|s| !s.is_empty())
        && let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(dm_v2)
        && let Ok(dm) = DmV2::decode(bytes.as_slice())
    {
        // Skip survive scene.
        ...
        if DmType::try_from(dm.dm_type).ok() == Some(DmType::Emoticon)
            && let Some(e) = dm.emoticons.iter().find_map(|x| x.value.as_ref())
        {
...
fn extract_wbi_key(url: &str) -> Option<String> {
    ...
    if let Ok(u) = url::Url::parse(url)
        && let Some(seg) = u.path_segments().and_then(|mut s| s.next_back())
    {
