...
use chaos_core::lyrics::providers::{
    KugouProvider, LrcLibProvider, NeteaseProvider, QqMusicProvider,
};
...
async fn qq_search_and_fetch_parses_jsonp_and_base64() {
    ...
    let p = QqMusicProvider::with_base_url(
        &format!("{}/search", server.base_url()),
        &format!("{}/lyric", server.base_url()),
    );
    let http = reqwest::Client::new();
    let req = req_info("Hello", "Adele", Some(296_000));

    let toks = p
        .search(&http, &req, Duration::from_millis(1000))
        .await
        .unwrap();
    assert_eq!(toks.len(), 1);

    let item = p
        .fetch(&http, toks[0].clone(), &req, Duration::from_millis(1000))
        .await
        .unwrap();
...
async fn kugou_fetch_decrypts_krc() {
    ...
    server.mock(|when, then| {
        ...
        then.status(200)
            .body(format!(r#"{{"content":"{content_b64}","fmt":"krc"}}"#));
    });

    let p = KugouProvider::with_base_url(
        &format!("{}/search", server.base_url()),
