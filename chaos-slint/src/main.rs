...
enum UiMsg {
    ...
    DanmakuDisconnected,
    DanmakuEvent(chaos_core::danmaku::model::DanmakuEvent),
    DanmakuImage {
        row_id: i32,
        w: u32,
        h: u32,
...
fn spawn_runtime_thread(
    ...
    std::thread::spawn(move || {
        ...
        struct ActiveDanmaku {
            session: chaos_core::danmaku::model::DanmakuSession,
            ...
            while let Some(msg) = task_rx.recv().await {
                match msg {
                    ...
                    TaskMsg::DanmakuConnect { input } => {
                        ...
                        let client = match chaos_core::danmaku::client::DanmakuClient::new() {
                            Ok(c) => c,
                            Err(e) => {
                        ...
                        let (session, mut rx) = match client
                            .connect_resolved(target, chaos_core::danmaku::model::ConnectOptions::default())
                            .await
                        {
                            Ok(v) => v,
                            Err(e) => {
