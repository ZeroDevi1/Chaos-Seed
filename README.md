...
- WinUI 3（PoC 已实现）：直播页输入直播间地址 → 解析展示清晰度/线路卡片 → 点击卡片播放（Flyleaf/FFmpeg）→ 右侧弹幕滚动（支持表情图片）；新增“歌词”页（Now Playing + 顺序多源阈值搜索，可选 daemon/FFI 后端）；新增“弹幕”页（独立连接/断开，不与直播会话互相干扰；支持 Chat 窗口与 Overlay 透明悬浮窗）
