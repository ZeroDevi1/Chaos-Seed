...
## P2-7 Setting：日志输出开关（下次做）

### 需求
- 在 Setting 页面提供开关：是否输出 `logs/app.log`（默认关闭或默认跟随 debug/release 策略，二选一）。

### 实现方向（概括）
- Setting 页新增一个 toggle。
- Rust 侧引入全局配置（内存态即可，持久化后续再做）控制 `log_line()` 是否真正写文件。
- 可选：默认策略
  - Debug 默认开启
  - Release 默认关闭（用户手动打开后再写）

### 验收标准（可交付）
- 关闭时不再写入 `logs/app.log`；开启后才写。
