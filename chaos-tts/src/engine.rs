...
use crate::pack::CosyVoicePack;
use crate::sampling::{SamplingConfig, sample_ras_next};
use crate::text::{PromptStrategy, resolve_tts_text_basic};
use crate::wav::{duration_ms, encode_wav_pcm16_mono, f32_to_pcm16_mono};
use crate::TtsError;
...
pub struct CosyVoiceEngine {
    ...
    llm_prefill: Arc<SimplePlan<TypedFact, Box<dyn TypedOp>>>,
    llm_decode: Arc<SimplePlan<TypedFact, Box<dyn TypedOp>>>,
    flow_infer: Arc<SimplePlan<TypedFact, Box<dyn TypedOp>>>,
    hift_infer: Arc<SimplePlan<TypedFact, Box<dyn TypedOp>>>,
}
...
impl CosyVoiceEngine {
    ...
    pub fn synthesize_sft(&self, params: &TtsSftParams) -> Result<TtsAudioResult, TtsError> {
        self.synthesize_sft_with_cancel(params, None)
    }

    pub fn synthesize_sft_with_cancel(
        &self,
        params: &TtsSftParams,
        cancel: Option<&AtomicBool>,
    ) -> Result<TtsAudioResult, TtsError> {
        ...
        if !input_ids.iter().any(|&x| x as u32 == cfg.end_of_prompt_token_id) {
            return Err(TtsError::InvalidArg(format!(
        ...
        if let Some(c) = cancel {
            if c.load(Ordering::Relaxed) {
                return Err(TtsError::InvalidArg("canceled".into()));
            }
        }
    ...
    fn llm_generate(
        ...
        // Note: CosyVoice3's stop_token_ids are `speech_token_size..speech_token_size+200`, so
        // `min_len` isn't strictly enforced upstream. We implement it anyway for stability.
...
) -> Result<Arc<SimplePlan<TypedFact, Box<dyn TypedOp>>>, TtsError> {
...
fn extract_last_logits(v: &TValue) -> Result<Vec<f32>, TtsError> {
    let view = v
        .to_array_view::<f32>()
...
fn time_scale_mel_linear(mel: &[f32], channels: usize, speed: f32) -> Result<Vec<f32>, TtsError> {
    ...
    for ch in 0..channels {
        for i in 0..new_t {
            let src_pos = (i as f32) * (t.saturating_sub(1) as f32) / (new_t.saturating_sub(1).max(1) as f32);
