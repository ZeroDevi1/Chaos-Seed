      ...
      body: ListView(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text('进入画中画时临时关闭弹幕，退出后恢复'),
            value: cur.pipHideDanmu,
            onChanged: (v) => s.update(cur.copyWith(pipHideDanmu: v)),
          ),
          ListTile(
            title: const Text('弹幕字号'),
            subtitle: Text(cur.danmuFontSize.toStringAsFixed(0)),
          ),
          Slider(
            value: cur.danmuFontSize,
            min: 12,
            max: 28,
            divisions: 16,
            onChanged: (v) => s.update(cur.copyWith(danmuFontSize: v)),
          ),
          ListTile(
            title: const Text('弹幕透明度'),
            subtitle: Text(cur.danmuOpacity.toStringAsFixed(2)),
          ),
          Slider(
            value: cur.danmuOpacity,
            min: 0.2,
            max: 1.0,
            divisions: 16,
            onChanged: (v) => s.update(cur.copyWith(danmuOpacity: v)),
          ),
          ListTile(
            title: const Text('弹幕区域（占屏高度）'),
            subtitle: Text(cur.danmuArea.toStringAsFixed(2)),
          ),
          Slider(
            value: cur.danmuArea,
            min: 0.2,
            max: 1.0,
            divisions: 16,
            onChanged: (v) => s.update(cur.copyWith(danmuArea: v)),
          ),
          ListTile(
            title: const Text('弹幕速度（秒/屏）'),
            subtitle: Text('${cur.danmuSpeedSeconds}'),
          ),
          Slider(
            value: cur.danmuSpeedSeconds.toDouble(),
            min: 4,
            max: 16,
            divisions: 12,
            onChanged: (v) =>
                s.update(cur.copyWith(danmuSpeedSeconds: v.round())),
          ),
          const Divider(),
